...
(function(){
	...
	var Attrs = {
		...
		showPage: function(nextIdx){
			...
			var _this = this;
				...
				animStyle = this.options.animStyle;
			...
			var isUp = nextIdx > idx ? true : false,
				top = isUp ? '100%' : '-100%',
				hideTop = isUp ? '-100%' : '100%',
				ac = isUp ? 'up' : 'down',
				anim = animStyle ? ('petrus-slide-anim-'+ac+'-' + animStyle) : 'default';

			$nextPage.css('z-index', 1).siblings().css('z-index', 0);

			var hideAnim = ('petrus-slide-anim-'+ac+'-hide-' + animStyle);

			$page.addClass(hideAnim);

			setTimeout(function(){
				$page.addClass('petrus-slide-hide').removeClass(hideAnim);
				// $page.css('top', hideTop).addClass('petrus-slide-hide').removeClass(hideAnim);
				if(options.callbacks.outEnd){
					options.callbacks.outEnd.call(null, idx, nextIdx);
				}
			}, 800);

			$nextPage.removeClass('petrus-slide-hide');

			$nextPage.addClass(anim);

			setTimeout(function(){
				// $nextPage.css('top', 0).removeClass(anim);
				$nextPage.removeClass(anim);
				options.idx = nextIdx;
	...
	var slide = function(settings){
		...
		if(!options.animStyle){
			animSpeed = options.speed || 800;
		}
